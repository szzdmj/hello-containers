...
//可能是images接口
if(substr($requestUri,0,8)=='/images/'){
...
if(empty($remoteUrl) && strpos($requestUri,'.m3u8')!==false && preg_match('#^/([vl]\w{30})(?:-(\d+)p)?\.m3u8#', $requestUri, $match)){
    ...
    */
    $error = '';
    $url = getGanjingVideoUrl($match[1], null, false, $error);
...
if(empty($remoteUrl) && ($pageidValues=$urlCoding->getPageIdUrl($requestUri))){
	...
	}elseif(strpos($url,'//www.szzd.org/')!==false){
        if(preg_match('#^https?://www\.szzd\.org(/(?:v\.php|faq\.php|mhr\.php|news\.php|video/|tui/|jump/).*)$#', $url, $m)){
...
if(empty($remoteUrl)){
    ...
    if($remoteUrl && (in_array($remoteUrl->host,['www.szzd.org','szzd.org']) || substr($remoteUrl->host,0,13)=='www.szzd.org.') && preg_match('#^/(\d+|\w{4,8})(\?\w+)?$#', $remoteUrl->uri)){
...
//针对特定404显示pwa页
if($remoteUrl->host=='www.szzd.org' && $remoteUrl->uri=='/404.htm'){
	$pwa->outputWebpage($currentUrl, $urlCoding);
}
